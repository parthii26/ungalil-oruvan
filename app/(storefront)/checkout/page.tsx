      ...
      <ul className="mt-4 space-y-3 text-sm">
        {cart.items.map((i) => (
          <li key={i.item_id} className="flex justify-between gap-3">
            ...
            <span className="whitespace-nowrap">{formatPrice(i.unit_price_paise * i.quantity)}</span>
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="container-page py-8 md:py-12 grid lg:grid-cols-[1fr_340px] gap-8 md:gap-12">
      <div className="min-w-0">
        ...
        <details className="mt-6 border border-line bg-warmwhite px-4 lg:hidden">
          <summary className="text-sm font-medium">
            Order summary · {formatPrice(cart.quote.grand_total_paise)}
          </summary>
      ...
      <aside className="hidden lg:block border border-line p-6 h-fit">
