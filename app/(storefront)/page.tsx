    ...
    <div>
      ...
      <section className="bg-warmwhite border-b border-line">
        <div className="container-page grid grid-cols-2 gap-y-1 py-4 md:grid-cols-5 md:py-0">
          {["Farm direct", "Small lots", "Honest weights", "Short lists", "No theatre"].map((item) => (
      ...
      <section id="story" className="grid md:grid-cols-2 md:min-h-[70vh]">
        ...
        <div className="flex flex-col justify-center px-6 md:px-16 py-12 md:py-16 bg-forest text-cream">
          ...
          <p className="mt-6 text-cream/80 leading-relaxed max-w-md">
            Soil first. Grain, oil, and spice named to the farm — not a slogan. Packed in small lots so the jar still remembers the season.
          </p>
        </div>
      </section>
      ...
      <section className="grid md:grid-cols-2 md:min-h-[70vh]">
        <div className="order-2 md:order-1 flex flex-col justify-center px-6 md:px-16 py-12 md:py-16 bg-cream">
          ...
          <p className="mt-6 text-[0.7rem] tracking-[0.16em] uppercase text-earth">Crop, method, and harvest — on the product</p>
        </div>
        <EditorialImage
      ...
      <section className="container-page py-12 md:py-20">
        ...
        <div className="mt-8 md:mt-10 grid grid-cols-2 md:grid-cols-4 gap-3">
          {categories.map((c) => (
            <Link key={c.id} href={`/category/${c.slug}`} className="group relative aspect-[4/5] overflow-hidden bg-paper-deep" data-hint="View product">
              {c.image_path && (
                ...
                <img src={c.image_path} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.03] group-hover:translate-x-[1%]" />
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-charcoal/70 via-transparent to-transparent" />
              <p className="absolute bottom-3 left-3 right-3 md:bottom-4 md:left-4 md:right-4 text-cream font-serif text-xl md:text-2xl leading-tight">{c.name}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="bg-warmwhite py-12 md:py-16">
        <div className="container-page grid grid-cols-3 gap-4 md:gap-8">
          <CountUp value={published.length} label="Products in the pantry" />
          <CountUp value={categories.length} label="Categories" />
          <CountUp value={loadDb().product_variants.length} label="Packs & sizes" />
        </div>
      </section>

      <section className="container-page py-12 md:py-16">
        <h2 className="font-serif text-3xl md:text-4xl text-forest">Best sellers</h2>
        <div className="mt-8 md:mt-10 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {sellers.map((c) => (
            <ProductCard key={c.product.id} card={c} />
          ))}
      ...
      <section className="bg-warmwhite py-12 md:py-20">
        <div className="container-page grid md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div>
            <p className="label">Trust</p>
            <h2 className="font-serif text-3xl md:text-4xl text-forest mt-2">On the product, not the wallpaper</h2>
          ...
          <ul className="space-y-4 text-sm">
            ...
            <li>Traceability is a label: farm, crop, process</li>
          </ul>
        </div>
      </section>
