          ...
          {STEPS.map((s) => (
            <article key={s.k} className="min-w-[200px] snap-start" data-cursor="explore">
              ...
              <div className="space-y-0.5 mt-3">
                <p className="font-tamil text-base md:text-lg font-medium text-forest">{s.ta}</p>
                <p className="font-serif text-base md:text-lg font-normal text-ink">{s.k}</p>
              </div>
              <p className="mt-1 text-xs text-ink-soft">{s.e}</p>
            </article>
          ))}
        </div>
      </div>
